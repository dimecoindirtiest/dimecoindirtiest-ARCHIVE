...
uint256 hashGenesisBlock("0x00000c31cbfa287f2bc7c6c5634475883af72c6dd47cd3d27341bc668f731c81");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // dimecoindirtiest: starting difficulty is 1 / 2^12
...
int64 CTransaction::nMinTxFee = 0.01 * COIN;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 0.01 * COIN;
...
const string strMessageMagic = "dimecoindirtiest Signed Message:\n";
...
{
    // dimecoindirtiest: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight == 1)
		return 2325000000 * COIN;

    nSubsidy >>= (nHeight / 7500000);
...
{
    // dimecoindirtiest: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		const char* pszTimestamp = "The Dirtiest Yet";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xccdb4201639fe6cf353f4f79fb81678bf0f53fad4c998f7416948e2eef592e71"));
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc1, 0xb1, 0xde }; // dimecoindirtiest: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("dimecoindirtiestMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("dimecoindirtiestMiner : ProcessBlock, block not accepted");
    }
